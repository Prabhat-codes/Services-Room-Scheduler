...
import { getBookings, getBuildings, getCompanyForEdit, getPresets } from "@/server/admin-data";
...
export default async function EditCompanyPage({ params }: PageProps<"/admin/companies/[id]/edit">) {
  ...
  if (!company) notFound();
  const [presets, buildings, bookings] = await Promise.all([getPresets(), getBuildings({ activeOnly: true }), getBookings()]);
  return (
    <Page className="max-w-5xl">
      ...
      />
      <CompanyForm presets={presets} buildings={buildings} bookings={bookings} initial={company} />
    </Page>
  );
}
